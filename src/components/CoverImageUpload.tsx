...
const CoverImageModal = () => {
  ...
  const handleFileChange = async (
    ...
    const imageUrl = `${supabaseUrl}/storage/v1/object/public/profile-pictures/cover-images/${fileName}`;
  ...
  const handleUpdateFileChange = async (
    ...
    try {
      ...
      const filePath = `cover-images/${fileName}`; // Define file path in storage
      ...
      const currentProfileImage = user.user_metadata.coverimage; // Get the current profile image URL from user metadata
      if (currentProfileImage) {
        const oldFileName = currentProfileImage.split("/").pop(); // Extract the old file name from the URL
        const { error: deleteError } = await supabase.storage
          .from("profile-pictures")
          .remove([`cover-images/${oldFileName}`]); // Delete the old image
      ...
      const { error: authError } = await supabase.auth.updateUser({
        data: { coverimage: imageUrl }, // Update user metadata
      });
  ...
  const removeProfileImage = async () => {
    ...
    try {
      ...
      const { error: authError } = await supabase.auth.updateUser({
        data: { coverimage: "" }, // Reset to default
      });
      ...
      const currentProfileImage = user.user_metadata.coverimage; // Make sure to get the current profile image URL
      if (currentProfileImage) {
        const fileName = currentProfileImage.split("/").pop(); // Extract the file name from the URL
        const { error: storageError } = await supabase.storage
          .from("profile-pictures")
          .remove([`cover-images/${fileName}`]); // Remove the image from storage
          ...
          <Dialog.Content margin={10}>
            ...
            <Dialog.Body>
              <div className="flex-col flex justify-center items-center">
                <div className={user?.user_metadata.coverimage ? "border border-white" :""}>
                  {user?.user_metadata.coverimage ? (
                    <img src={user?.user_metadata.coverimage} />
                  ) : (
                    "Upload Cover Image"
                  )}
                </div>
